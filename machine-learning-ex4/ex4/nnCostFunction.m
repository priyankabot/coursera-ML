...
for i = 1:m
	Xi = X(i,:);
	a = sigmoid(Xi * Theta1');
	hyp = sigmoid( [ 1 a] * Theta2');
        yi = zeros(1,k);
        yi(y(i)) = 1;
        J = J + sum( -1 * yi .* log(hyp) - (1 - yi) .* log(1 - hyp));	
end;
...
delta_sum1 = zeros(size(Theta1));
delta_sum2 = zeros(size(Theta2));
...
for i = 1:m
	...
	delta2 = delta3*Theta2 .* (a2 .* (1-a2));
        delta_sum1 = delta_sum1 + delta2(2:end)' * a1;
	delta_sum2 = delta_sum2 + delta3' * a2;
